...
2. File-level: Check if file already exists locally (archive + title match)
"""
...
class DuplicateFilter:
    ...
    def is_file_downloaded(self, video_id: str, directory: str, title: str = None,
                           _existing_titles: Optional[Set[str]] = None) -> bool:
        """
        ...
        try:
            ...
            if not file_exists and title:
                ...
                if normalized_title:
                    titles = _existing_titles if _existing_titles is not None else self._get_existing_titles(directory)
    ...
    def filter_already_downloaded(
        ...
        for video in videos:
            ...
            if self.is_file_downloaded(video_id, download_directory, title=video_title,
                                       _existing_titles=existing_titles):
                skipped += 1
