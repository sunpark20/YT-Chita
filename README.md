...
바다에서 근무합니다. 오프라인 환경이라 한번에 동영상을 다운받아가요.
여행유튜브는 동영상이 아주 많기 때문에 순서대로 보기 쉽지 않아요.
...
- **채널 전체 다운로드** — 채널의 모든 영상 전부다
- **재생목록 다운로드** — 단일 재생목록 또는 채널의 모든 재생목록의 모든 영상 전부다
- **개별 다운로드** — 단일 URL 입력으로 1개 가능
...
- **실리콘맥 우선** — 제게 인텔맥과 윈도우는 없기 때문에 테스트가 너무 어렵습니다. 댓글이나 이메일로 문제 보내주시면, 시간날 때 개선하겠습니다.
