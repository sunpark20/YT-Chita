## 소개

제가 쓰려고 만들었어요. 4K Downloader 영구 라이센스 유료로 쓰고 있었는데, 이놈들이 구버전 서비스 종료하고 신버전을 만들더라구요. 열받아!!!(아주 과거임)
바다에서 근무합니다. 오프라인 환경이라 한번에 동영상을 다운받아가요.
여행유튜브는 동영상이 아주 많기 때문에 순서대로 보기 쉽지 않아요.
그래서 다운받아지면 `260311_영상제목` 같이 정렬되게 만들었어요.

## 지원기기

맥(실리콘칩,인텔칩), 윈도우10_11(64bit)/32bit는 불가

## 주요 기능

- **채널 전체 다운로드** — 채널의 모든 영상 전부다
- **재생목록 다운로드** — 단일 재생목록 또는 채널의 모든 재생목록의 모든 영상 전부다
- **개별 다운로드** — 단일 URL 입력으로 1개 가능
- **화질 선택** — 음원만(m4a) / 360p / 720p / 1080p / 최고화질

## 다운로드 및 업데이트 현황

https://github.com/sunpark20/YT-Chita/releases


## 오류 신고

앱 사용 중 크래시가 발생하면 **자동으로 크래시 리포트가 전송**됩니다. 별도 조치 없이도 제가 확인할 수 있으니 개선하겠습니다.

아래와 같이 앱이 아예 실행되지 않거나, 크래시 없이 이상 동작하는 경우에는 직접 신고해 주세요.

- **빌드 버전** — 다운로드한 파일의 버전 (예: `v1.2.9`)
- **증상 설명** — 구체적으로 어떤 상황인지 (예: 프로그램을 눌렀는데 안 뜸 / 떴다가 바로 사라짐 / 특정 오류 메시지 등)
- **PC 사양** — OS 버전, CPU, RAM 등 기본 스펙 스크린샷

신고 이메일: **sun.park20@gmail.com**

## 한계

- **실리콘맥 우선** — 제게 인텔맥과 윈도우는 없기 때문에 테스트가 너무 어렵습니다.
댓글이나 이메일로 문제 보내주시면, 시간날 때 개선하겠습니다.
- **동작 문제없으나 고칠점** — [개선필요 목록](docs/개선필요.md), 1.업데이트 플로우를 어떻게 개선함???? 인터넷 다운로드, 2.자막 넣기 빼기 -다운로드 5000개 요청
깃허브에서 릴리즈되는 파일에 마지막에 v1.3.0 과 같이 버전붙이기.
크래쉬리포트 1.3.0 고치기

   
## 포맷

- **음원(m4a)**: AAC 원본 그대로 저장(변환 없음)
- **360p / 720p / 1080p**: H.264 우선, 없으면 다른 코덱으로 대체
- **최고화질**: 코덱 무관, 최고 해상도 다운로드 (4K VP9/AV1 등)


## 참고

- API 키 추가하면, 오류 덜 나고 영상 분석이 빨라집니다. 100개 이상 영상 받을 때 추천


## Disclaimer

이 앱은 개인 오프라인 시청, 백업 등 합법적 용도를 위해 만들어졌습니다. 저작권이 있는 콘텐츠의 무단 배포는 사용자의 책임입니다.


## License

이 프로젝트는 [The Unlicense](https://unlicense.org/)로 배포됩니다.

### Open Source Libraries

| 라이브러리 | 라이선스 |
|-----------|---------|
| [yt-dlp](https://github.com/yt-dlp/yt-dlp) | Unlicense |
| [FFmpeg](https://ffmpeg.org/) | LGPL-2.1 / GPL |
| [FastAPI](https://github.com/tiangolo/fastapi) | MIT |
| [pywebview](https://github.com/nicegui-kr/pywebview) | BSD-3-Clause |
| [uvicorn](https://github.com/encode/uvicorn) | BSD-3-Clause |
| [Pydantic](https://github.com/pydantic/pydantic) | MIT |
| [PyInstaller](https://github.com/pyinstaller/pyinstaller) | GPL-2.0 (빌드 도구) |
